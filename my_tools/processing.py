...
SR = FPS * HOP_LENGTH
EPS = 1e-6
CACHE_DIR = "/data00/lizhengjia/transflower-lightning/music_features"
...
def music_features_all(path, tempo=120.0, concat=False):
    cache_path = os.path.join(CACHE_DIR, os.path.basename(path).replace(".wav", ".pkl"))
...
def music_tempogram(path=None, envelope=None, win_length=384):
    ...
    tempogram = librosa.feature.tempogram(
        onset_envelope=envelope.flatten(), sr=SR, hop_length=HOP_LENGTH, win_length=win_length
    )
...
def music_chroma(path=None, data=None, n_chroma=12):
    ...
    if data is None:
        data = music_load(path)
    chroma = librosa.feature.chroma_cens(data, sr=SR, hop_length=HOP_LENGTH, n_chroma=n_chroma)
...
def music_beat_onehot(path=None, envelope=None, start_bpm=120.0, tightness=100):
...
def motion_peak_onehot(joints):
    ...
    peak_idxs = scisignal.argrelextrema(
        envelope, np.less, axis=0, order=int(FPS * 10 / 60)
    )  # order=10 for 60FPS
    peak_onehot = np.zeros_like(envelope, dtype=bool)
    peak_onehot[peak_idxs] = 1
...
def select_aligned(music_beats, motion_beats, tol=6):
    ...
    if len(music_beat_idxs) == 0 or len(motion_beat_idxs) == 0:
...
def alignment_score(music_beats, motion_beats, sigma=3):
    ...
    if motion_beats.sum() == 0 or music_beats.sum() == 0:
        return 0.0
...
def calculate_metrics(
    ...
    for music_path, joints, bpm in tqdm.tqdm(zip(music_paths, joints_list, bpm_list)):
        ...
        metrics["beat_alignment"].append(alignment_score(music_beats, motion_beats, sigma=sigma))
